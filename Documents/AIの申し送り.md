...
| 2026/9/10 | Claude Code | 「重力を時間で変える」と言われて、`Physics.gravity` を書き換えた | **人もロボットもカートも、まったく影響を受けなかった。** これらは `CharacterController` で動いていて、**落ちる速さを各スクリプトが自分で計算している**ため。`Physics.gravity` が効くのは Rigidbody で動く物だけ | **「Unityの設定を変えれば効く」と思い込まない。** まず `grep -rn "gravity" --include=*.cs` で、**自前で重力を計算している場所を数える**こと。共通の入れ物（`Assets/Scripts/Common/WorldGravity.cs`）を1つ作り、**計算している場所が全部そこを見る**形にするのが確実。`Physics.gravity` も一緒に書き換えておけば、転がる物にも効く |
