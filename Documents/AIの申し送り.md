...
| 2026/9/3 | Claude Code | `perl -0777 -i -pe` の置換文字列の中で、Perlの式（`$変数 . "文字列"`）を書いた | **置換文字列はただの文字列として扱われるため、書いた式がそのままファイルに入った。** `README.md` に ` . "` という行が紛れ込んだ。**同じ日に2回やった** | **置換文字列の中で計算しない。** 足したい文字列は、置換する前に変数へ組み立ててから `s{...}{$変数}` の形で使う。**あるいは最初からファイル編集用のツールを使う**（そのほうが速くて安全）。書き換えたあとは `grep` で残骸（` . "` や行頭の `"`）が無いか必ず確かめる |
