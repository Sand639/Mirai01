...
| 2026/9/3 | Claude Code | 日本語のドキュメントを書いていた | **日本語の文章の中に、キリル文字（ロシア語の文字）が混ざった。** 「判断材料」と書くつもりが「判断материал」になっていた。見た目では気づきにくい。**2026/9/1にも同じことをしている**（`колиダー` `материал` `влияние`） | **書いたあとに `grep -rlP '[\x{0400}-\x{04FF}]' Documents/ Mirai01/Assets/Scripts/` を実行して確かめる。** 0件なら綺麗。日本語の文章を長く書いたときは、毎回これを通すこと |
